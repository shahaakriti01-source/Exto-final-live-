...
/**
 * All unique REAL merchants — determined ENTIRELY from onboarding events, never
 * from payments. A person's presence as a sender/receiver in a payment transaction
 * does NOT make them a merchant — only being onboarded by Exto/a field coordinator
 * (or onboarding at least one consumer themselves) does. This is critical: a real
 * consumer who simply appears in a payment (e.g. receiving money from the merchant
 * who onboarded them) must NEVER be promoted into the merchant list just for that.
 */
...
/** All consumer names — anyone onboarded who is NOT a merchant. Used to make sure a
 *  consumer can never also be misclassified or treated as a merchant downstream. */
...
/** Every consumer, paired with the name of the merchant who onboarded them
 *  (deduped by Owner, same as onboarding events generally). Used by the
 *  Consumers tab for a flat, unambiguous list distinct from merchants. */
