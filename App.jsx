...
import { dedupeOnboardingEvents, getAllMerchantNames, getConsumersForMerchant, getAllConsumersWithMerchant, SPECIAL_ACCOUNT_NAMES, isSpecialAccountName } from "./onboardingLogic";
...
function Dashboard({ onboardingEvents, payments, dataSource, lastUpdated, onRefresh, refreshing, onLogout }) {
  ...
  const [visibleKPIs, setVisibleKPIs] = useState({ merchants: true, consumers: true, qualifying: true, volume: true });
  ...
  const merchantNames = useMemo(() => getAllMerchantNames(incOnboarding), [incOnboarding, incPayments]);
  ...
  const dashMerchantNames = useMemo(() => getAllMerchantNames(dashOnboarding), [dashOnboarding, dashPayments]);
  ...
  const triggersMerchantNames = useMemo(() => getAllMerchantNames(triggersOnboarding), [triggersOnboarding, triggersPayments]);
  ...
  const merchantsMerchantNames = useMemo(() => getAllMerchantNames(merchantsOnboarding), [merchantsOnboarding, merchantsPayments]);
  ...
  const auditMerchantNamesForTab = useMemo(() => getAllMerchantNames(auditOnboarding), [auditOnboarding, auditPayments]);
  ...
  const dashStats = useMemo(() => {
    ...
    dashMerchantNames.forEach(m => {
      ...
      // Volume now matches the EXACT same "qualifying transactions" used for
      // incentive calculations everywhere else — net of reversed Revokable/
      // Returnable pairs, and only transactions involving a real merchant
      // (never special-account-to-special-account activity).
      r.qualifyingTransactions.forEach(p => {
        if (countedTxnIds.has(p.txn_id)) return; // already added via the other merchant in a merchant-to-merchant txn
  ...
  const onboardTrend = useMemo(() => {
    ...
    return Object.entries(byDay).sort(([a], [b]) => a.localeCompare(b))
      .map(([, v]) => ({ day: v.label, merchants: v.merchants.size, consumers: v.consumers.size }));
  }, [dashOnboarding]);
        ...
        {tab === "dashboard" && (
          <div>
            ...
            <div style={{ display: "flex", gap: 14, marginBottom: 20, flexWrap: "wrap" }}>
              ...
              {!visibleKPIs.merchants && !visibleKPIs.consumers && !visibleKPIs.qualifying && !visibleKPIs.volume && (
            ...
            {paidList.length > 0 && (
              <div style={{ marginTop: 20 }}>
                <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 10 }}>
                  ...
                  <button onClick={() => downloadPaidHistoryCSV(paidList, triggersPhase, triggersDateFrom, triggersDateTo)} style={{ background: "transparent", color: C.gold, border: `1px solid ${C.gold}55`, borderRadius: 7, padding: "5px 12px", fontSize: 12, fontWeight: 600, cursor: "pointer" }}>
                    ⬇ Download CSV
                  </button>
                </div>
                {paidList.map(t => (
                  ...
                  {expandedConsumer === (c.owner || idx) && (
                    <div style={{ background: C.bgDark, padding: 16, borderTop: `1px solid ${C.border}` }}>
                      <div style={{ display: "grid", gridTemplateColumns: "repeat(4, 1fr)", gap: 8, fontSize: 12, marginBottom: 14 }}>
                        ...
                        <div><div style={{ color: C.textFaint }}>Variable</div><div style={{ color: C.gold, fontWeight: 700 }}>BWP {c.variableIncentive}</div></div>
